...
class MY_Model extends CI_Model{
  ...
  /**
   * Conecta ao banco de dados dos clientes da Sillycomm.
   *
   * Necessario informar o id do cliente para consulta do cliente
   *
   * @var int $id_cliente
   * @return nome cliente
   * @return boolean
   * @return String
   */
  public function doConnectDbClient($id_cliente) {
    $this->load->library('encrypt');
    $db_radcheck    = $this->db->select('nome, servidor, servidor_banco, servidor_usuario, servidor_senha')->get_where('clientes', array('id_cliente' => $id_cliente))->row();
    if (!empty($db_radcheck)) 
    {
      $dsn            = 'mysqli://'.$db_radcheck->servidor_usuario.':'.$this->encrypt->decode($db_radcheck->servidor_senha).'@'.$db_radcheck->servidor.'/'.$db_radcheck->servidor_banco;
      $this->radcheck = $this->load->database($dsn, true);
      return $db_radcheck->nome;
    }
    return false;
  }
